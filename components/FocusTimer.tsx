  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 text-xs text-zinc-500">
        PID: 8392 (timer_daemon) | STATE: {isActive ? 'RUNNING' : 'STOPPED'}
      </div>
      ...
      <div className="z-10 text-center">
        <div className={`text-xs font-bold mb-4 tracking-[0.3em] uppercase ${isBreak ? 'text-blue-500' : 'text-green-500'}`}>
           *** {isBreak ? 'SYSTEM COOLING (BREAK)' : 'PROCESSING (FOCUS)'} ***
        </div>
        ...
        <div className="flex gap-4 justify-center">
          ...
          >
            {isActive ? 'SIGSTOP (PAUSE)' : 'SIGCONT (START)'}
          </button>
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-zinc-800 p-2 bg-zinc-950 text-[10px] font-mono text-zinc-500 h-24 overflow-hidden">
        <div>[LOG] Daemon started at {new Date().toLocaleTimeString()}</div>
        <div>[LOG] Mode set to {isBreak ? 'BREAK' : 'WORK'}</div>
        <div>[LOG] Sessions completed: {sessionCount}</div>
        <div>[LOG] Total uptime today: {initialStats.studyMinutesToday} minutes</div>
        {isActive && <div className="text-green-500 animate-pulse">[LOG] Timer tick...</div>}
      </div>
