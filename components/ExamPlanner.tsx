  ...
  return (
    <div className="font-mono text-sm space-y-6">
      ...
      <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
        <h2 className="text-green-500 font-bold text-lg">/etc/exams.conf</h2>
        <button
          onClick={() => {
      ...
      {isAdding && (
        <div className="border border-dashed border-zinc-700 p-6 bg-zinc-900/30">
          <div className="mb-4 text-zinc-500 text-xs">
            {editingId ? `# Editing configuration for ${editingId}` : '# Add new exam configuration'}
          </div>
          <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
            ...
            <div className="flex flex-col gap-1">
               <label className="text-zinc-400 text-xs">DUE_DATE=</label>
               <input
                type="date"
                value={date}
            ...
            <div className="flex flex-col gap-1">
               <label className="text-zinc-400 text-xs">DIFFICULTY_LEVEL=</label>
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <div className="text-zinc-500 text-xs font-mono mt-2">
                    Plan_Progress: [{ '#'.repeat(progress) }{ '.'.repeat(20 - progress) }]
                    <span className="ml-2 text-zinc-400">{completedSessions}/{totalSessions} modules</span>
                  </div>
                </div>
                ...
                <div className="flex flex-row md:flex-col gap-2 items-end">
                   ...
                   >
                     {loadingId === exam.id ? 'generating_plan...' : './generate_plan.sh'}
                   </button>
                   ...
                   <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                     ...
                     >
                       nano {exam.subject.substring(0,3).toLowerCase()}.conf
                     </button>
